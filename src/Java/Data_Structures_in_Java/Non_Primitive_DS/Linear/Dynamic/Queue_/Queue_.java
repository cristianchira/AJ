 ...
 * {@code Queue} implementations; in most implementations, insert operations cannot fail.
 *
 ...
 * </table>
 *
 * <p>Queues typically, but do not necessarily, order elements in a FIFO (first-in-first-out) manner.Among the exceptions are
 ...
 * Whatever the ordering used, the <em>head</em> of the queue is that element which would be removed by a call to
 * {@link #remove() } or {@link #poll()}.  In a FIFO queue, all new elements are inserted at the <em>tail</em> of the queue.
 * Other kinds of queues may use different placement rules.  Every {@code Queue} implementation
 * must specify its ordering properties.
 *
 * <p>The {@link #offer offer} method inserts an element if possible, otherwise returning {@code false}. This differs from
 * the {@link Collection#add Collection.add} method, which can fail to add an element only by throwing an unchecked exception.
