package Concurency_.Lock_and_Unlock;
/*
*                               ==Lock==
*
...
*
1) A synchronized block is fully contained within a method – we can have Lock API's lock() and unlock() operation in separate methods
2) A synchronized block doesn't support the fairness, any thread can acquire the lock once released, no preference can be specified.
...
4)A thread gets blocked if it can't get an access to the synchronized block.
...
5)A thread which is in “waiting” state to acquire the access to synchronized block, can't be interrupted.
...
import static Concurency_.Lock_and_Unlock.Main.EOF;
